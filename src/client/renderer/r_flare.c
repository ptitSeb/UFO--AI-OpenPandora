...
#define __default_texcoords__ default_texcoords2
#define GL_PRIMITIVE	GL_TRIANGLES
#else
#define __sommets__	4
#define __default_texcoords__ default_texcoords
#define GL_PRIMITIVE	GL_QUADS
#endif
	vec3_t view, verts[__sommets__];
	...
	for (i = 0; i < surfs->count; i++) {
		...
		if (f->image != image) {
			glDrawArrays(GL_PRIMITIVE, 0, l / 3);
			j = k = l = 0;
...
#ifdef HAVE_GLES
		// spilt QUADS in 2 triangles
		memcpy(&verts[4], &verts[3], sizeof(vec3_t));
		memcpy(&verts[3], &verts[2], sizeof(vec3_t));
		memcpy(&verts[5], &verts[0], sizeof(vec3_t));
#endif

		for (m = 0; m < __sommets__; m++) { /* duplicate color data to all 4 or 6 verts */
		...
		/* copy texcoord info */
		memcpy(&texunit_diffuse.texcoord_array[k], __default_texcoords__, sizeof(vec2_t) * __sommets__);
		k += sizeof(vec2_t) / sizeof(vec_t) * __sommets__;

		/* and lastly copy the 4 verts */
		memcpy(&r_state.vertex_array_3d[l], verts, sizeof(vec3_t) * __sommets__);
		l += sizeof(vec3_t) / sizeof(vec_t) * __sommets__;
	}
